...
def create_or_upload_test_based_on_count(dojo_token, url, product_name, engagement_name, filename):
    ...
    # Verifica a existência de testes com o scan_type 'Dependency Check Scan' no engagement
    ...
    dpcheck_test_exists = any(test['test_type_name'] == "Dependency Check Scan" for test in tests_data.get('results', []))

    if not dpcheck_test_exists:
        # Se não existir um teste com 'Dependency Check', cria um novo teste
        print("Criando um novo teste com scan_type 'Dependency Check Scan'.")
        new_test_data = {
            "engagement": engagement_id,
            "scan_type": "Dependency Check Scan",
            "description": "Dependency Check Scan",
...
def uploadToDefectDojo(is_new_import, dojo_token, url, product_name, engagement_name, filename):
    ...
    files = {
        'file': (filename, open(filename, 'rb'), "text/xml")
    }
...
if __name__ == "__main__":
    if len(sys.argv) == 9:
        ...
        print('Uso: python3 dpcheck.py --product NOME_DO_PRODUTO --engagement NOME_DO_ENGAGEMENT --report REPORT_SCAN --dojo_token TOKEN_DO_DOJO')